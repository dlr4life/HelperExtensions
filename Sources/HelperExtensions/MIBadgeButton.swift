...
open class MIBadgeButton: UIButton {
    ...
    // This value will be calculate like: (Badge Label Height) / (this value)
    @IBInspectable
    ...
    // Badge's anchor. TopLeft, TopRight, BottomLeft, BottomRight and Center. Offset is required depend on anchor. Assign 0.0 if don't need offset
...
//    }
    ...
    fileprivate func setupBadgeViewWithString(badgeText: String?){
        badgeLabel.clipsToBounds = true
        badgeLabel.text = badgeText
        badgeLabel.font = UIFont(name: "SF-Pro", size: 12) // UIFont.systemFont(ofSize: 12)
        badgeLabel.textAlignment = .center
        badgeLabel.sizeToFit()
    ...
    // Calculate badge's X Y position. Offset are taking into count
